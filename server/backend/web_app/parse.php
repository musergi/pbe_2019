...
function parse_request($query) {
    ...
    $where_statment = parse_where($table_name, $query_parts);
...
function parse_where($table, $fragments) {
    $restrictions = array();
    ...
    add_spetial_restrictions($table, $restrictions);
    return !empty($restrictions) ? ' WHERE ' . implode(' AND ', $restrictions) : '';
}
function add_spetial_restrictions($table, &$restrictions) {
    if ($table == "tasks") array_push($restrictions, "date>NOW()");
    if ($table == "marks") array_push($restrictions, "student=" . $_SESSION['student']);
}
